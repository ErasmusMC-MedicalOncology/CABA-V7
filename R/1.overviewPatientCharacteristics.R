...
data.Swimmers <- data.Swimmers %>%
    dplyr::mutate(`Subject Number` = factor(`Subject Number`, levels = data.Swimmers %>% dplyr::arrange(`Date: Start of Post-Treatment`) %>% dplyr::pull(`Subject Number`) %>% rev)) %>%
...
plot.Swimmer <- data.Swimmers %>%
    ...
    ggplot2::geom_linerange(data = data.Swimmers %>% dplyr::filter(!is.na(`Date: Death`)), aes(xmin = `Date: End of Post-Treatment`, xmax = `Date: Death`), size = .25, alpha = .75, color = 'black', lty = '11') +

    # Total cabazitaxel treatment.
    ggplot2::geom_segment(aes(x = `Date: Start of Post-Treatment`, xend = `Date: End of Post-Treatment`), alpha = 0.5, size = 1, color = 'royalblue2') +
    ggplot2::geom_point(aes(x = `Date: Start of Post-Treatment`), size = 1.5, color = 'navy') +
    ggplot2::geom_point(aes(x = `Date: End of Post-Treatment`), size = 1.5, color = 'navy', fill = 'white', shape = 21) +
